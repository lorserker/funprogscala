...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeeds: List[String] = columnNames.filter(name => 
      name.startsWith("t01") ||
      name.startsWith("t03") ||
      name.startsWith("t11") ||
    ...
    val working: List[String] = columnNames.filter(name =>
      name.startsWith("t05") || name.startsWith("t1805"))

    val leisure: List[String] = columnNames.filter(name => !primaryNeeds.contains(name) && !working.contains(name))
      .filter(name =>
        name.startsWith("t02") ||
        name.startsWith("t04") ||
        name.startsWith("t06") ||
    ...
    (primaryNeeds.map(col), working.map(col), leisure.map(col))
  }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(row => 
      TimeUsageRow(
        working = row.getAs[String](0),
        sex = row.getAs[String](1),
        age = row.getAs[String](2),
        primaryNeeds = row.getAs[Double](3),
        work = row.getAs[Double](4),
        other = row.getAs[Double](5)))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey((turow: TimeUsageRow) => (turow.working, turow.sex, turow.age))
