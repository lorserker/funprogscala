...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float = if (mass > 0 )(
      ...
      ne.massX * ne.mass + 
      sw.massX * sw.mass + 
      se.massX * se.mass) / mass else centerX
    val massY: Float = if (mass > 0)(
      nw.massY * nw.mass + 
      ne.massY * ne.mass + 
      sw.massY * sw.mass + 
      se.massY * se.mass) / mass else centerY
  ...
  extends Quad {
    val (mass, massX, massY) = (
      ...
      bodies.map(_.x).sum / bodies.length : Float, 
      bodies.map(_.y).sum / bodies.length : Float
    )
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        def ins(f: Fork, b: Body): Fork = f insert b
        bodies.foldLeft(fork)(ins)
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      matrix(y*sectorPrecision + x) = apply(x, y) += b
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      while (x < sectorPrecision) {
        while (y < sectorPrecision) {
          result.matrix(y * sectorPrecision + x) = this(x, y) combine that(x, y)
          y += 1
        }
        x += 1
      }
