...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val clusters = points.map(p => (findClosest(p, means), p)).groupBy(_._1).map{ case (p, pairs) => (p, pairs.map(_._2))}
    val emptyClusters = means.toSet diff clusters.keySet
    clusters ++ emptyClusters.map((_, GenSeq.empty)).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    val clusterAverages = classified.map{ case (c, points) => (c, findAverage(c, points)) }
    oldMeans.map(p => clusterAverages(p))
  }
